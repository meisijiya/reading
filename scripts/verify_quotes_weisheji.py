#!/usr/bin/env python3
"""verify_quotes_weisheji.py — 验收《微服务设计（第2版）》知识包：
① 每个模块 README 的 📖原文引用必须 verbatim 命中对应 fulltext 文件；
② 卡片数（## §N）== toc.md 行数 == fulltext 文件数 == 16。
用法: python3 scripts/verify_quotes_weisheji.py
"""
...
def normalize(s: str) -> str:
    """NFKC + 弯引号折叠 + 去所有空白与 markdown 装饰符（*`#><），避免复制时不可见差异导致误报。"""
    s = unicodedata.normalize("NFKC", s).translate(_QUOTE_FOLD)
    return re.sub(r"[\s*`#><]+", "", s)
...
def main() -> int:
    ...
    for readme in sorted(BOOK.glob("[0-9][0-9]-*/README.md")):
        ...
        for anchor, q in quotes:
            nq = normalize(q)
            hit = any(nq and nq in text for text in corpus.values())
            if not hit:
                misses.append(anchor)
        status = "OK" if not misses else "FAIL"
        if misses:
            fail += 1
        print(f"[{status}] {module}: cards={len(cards)} quotes={len(quotes)} verbatim_miss={misses[:5]}")
    ...
    print(f"total quotes verified: {quotes_total}")
